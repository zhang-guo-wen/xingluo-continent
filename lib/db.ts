...
export async function createPost(post: Omit<PlazaPost, "id" | "createdAt" | "images"> & { images?: string[] }): Promise<PlazaPost> {
  const id = genId("post");
  ...
  if (DATABASE_URL) {
    ...
    return { ...post, images: post.images ?? [], id, createdAt };
  }
...
export async function getAllPosts(): Promise<PlazaPost[]> {
  if (DATABASE_URL) {
    ...
    return rows.map((r) => ({
      ...
      userAvatar: r.user_avatar, campId: r.camp_id ?? null, content: r.content, images: [], createdAt: r.created_at,
    }));
  }

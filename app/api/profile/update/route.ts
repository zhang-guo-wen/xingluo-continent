...
export async function PUT(request: NextRequest) {
  ...
  const { userId, userName, name, occupation, description, walletAddress, spaceUrl } = await request.json();
  if (!userId) return NextResponse.json({ error: "缺少 userId" }, { status: 400 });

  const uName = userName ?? name ?? "用户";

  const user = await updateUserProfile(userId, { name, occupation, description, walletAddress, spaceUrl });

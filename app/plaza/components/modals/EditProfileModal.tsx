...
interface Props {
  initial: { name: string; occupation: string; description: string; walletAddress: string };
  onSubmit: (data: { name: string; occupation: string; description: string; walletAddress: string }) => Promise<void>;
  onClose: () => void;
}
...
export default function EditProfileModal({ initial, onSubmit, onClose }: Props) {
  ...
  async function handleSave() {
    setSaving(true);
    try {
      await onSubmit({ name, occupation, description, walletAddress: wallet });
    } finally {
      setSaving(false);
    }
  }
